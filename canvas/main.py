...
async def stream_generator(prompt: str, images: Optional[List[str]], target_lang: str, uid: str, plan: str, user_ref):
    ...
    # 1. Primary: Bluesminds (Using Gemini Flash Lite Preview)
    try:
        ...
        print(f"Bluesminds (Gemini) Stream Failed: {e}")

    # 2. Secondary: Fireworks (Kimi Code fallback)
    if not has_streamed:
        try:
    ...
    # 3. Tertiary: Groq (Text Only fallback)
    ...
    if not has_streamed:
        try:
            ...
            print(f"Native Gemini Stream Failed: {e}")
